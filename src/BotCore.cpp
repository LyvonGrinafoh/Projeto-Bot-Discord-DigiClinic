...
void BotCore::enviarLembretes() {
    ...
    for (const auto& [id_usuario, lista_solicitacoes] : solicitacoes_por_usuario) {
        ...
        if (!lista_solicitacoes.empty()) {
            std::string conteudo_dm = (const char*)u8"Olá! 👋 Este é um lembrete das suas pendências ativas:\n\n";
            for (const auto& solicitacao : lista_solicitacoes) {
                std::string tipo_str = (solicitacao.tipo == DEMANDA) ? "Demanda" : "Lembrete";
                conteudo_dm += std::string((const char*)u8"**") + tipo_str + ":**\n";
                conteudo_dm += std::string((const char*)u8"**Código:** `") + std::to_string(solicitacao.id) + "`\n";
                conteudo_dm += std::string((const char*)u8"**Descrição:** ") + solicitacao.texto + "\n";
                conteudo_dm += std::string((const char*)u8"**Prazo:** ") + solicitacao.prazo + "\n\n";
            }
            conteudo_dm += (const char*)u8"Para finalizar, use o comando correspondente no servidor (`/finalizar_demanda` ou `/finalizar_lembrete`).";
...
void BotCore::signalHandler(int signum) {
    std::string msg = "Sinal " + std::to_string(signum) + " recebido. Para desligar o bot, digite 'stop' e pressione Enter.";
    std::cout << "\n" << msg << "\n> " << std::flush;
