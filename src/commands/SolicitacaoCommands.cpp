...
void SolicitacaoCommands::handle_demanda_pedido(const dpp::slashcommand_t& event) {
    ...
    auto finish_solicitacao_creation = [this, event, nova_solicitacao, is_pedido, responsavel](const std::string& anexo_filename_for_embed) mutable {
        if (db_.addOrUpdateSolicitacao(nova_solicitacao)) {
            dpp::embed embed = dpp::embed().set_color(is_pedido ? dpp::colors::light_sea_green : dpp::colors::orange).set_title(is_pedido ? "📢 Novo Pedido Criado" : "❗ Nova Demanda Criada").add_field("Código", std::to_string(nova_solicitacao.id), false).add_field("Responsável", responsavel.get_mention(), true).add_field("Criado por", event.command.get_issuing_user().get_mention(), true);
            ...
            if (!nova_solicitacao.anexo_path.empty()) {
                try {
                    ...
                    Utils::log_to_file("Erro ao ler arquivo de imagem (demanda anexo): " + std::string(e.what()));
                }
            }
            ...
            dpp::embed dm_embed = dpp::embed().set_color(is_pedido ? dpp::colors::light_sea_green : dpp::colors::orange).set_title(is_pedido ? "📬 Você recebeu um novo pedido!" : "📬 Você recebeu uma nova demanda!").add_field("Código", std::to_string(nova_solicitacao.id), false).add_field(is_pedido ? "Pedido" : "Demanda", nova_solicitacao.texto, false);
    ...
    if (!is_pedido && std::holds_alternative<dpp::snowflake>(param_anexo)) {
        ...
        Utils::BaixarAnexo(&bot_, anexo.url, caminho_salvar,
            [this, event, nova_solicitacao, anexo_filename, finish_solicitacao_creation, caminho_salvar](bool sucesso) mutable {
                if (sucesso) {
                    nova_solicitacao.anexo_path = caminho_salvar;
                    finish_solicitacao_creation(anexo_filename);
                }
                else {
                    event.edit_response(dpp::message("❌ Erro ao baixar o anexo. A solicitação não foi salva.").set_flags(dpp::m_ephemeral));
                }
            });
    }
    else {
...
void SolicitacaoCommands::handle_finalizar_solicitacao(const dpp::slashcommand_t& event) {
    ...
    if (std::holds_alternative<std::string>(obs_param)) {
        observacao = std::get<std::string>(obs_param);
    }
    ...
    if (item_opt) {
        ...
        if (command_name == "cancelar_pedido") {
            if (item_concluido.tipo != PEDIDO) {
                event.reply(dpp::message("❌ Este comando é apenas para cancelar **pedidos**.").set_flags(dpp::m_ephemeral));
                return;
            }
        }

        if (item_concluido.status != "pendente") {
            event.reply(dpp::message("❌ Esta solicitação não está mais pendente (Status: " + item_concluido.status + ").").set_flags(dpp::m_ephemeral));
            return;
        }
        ...
        if (db_.addOrUpdateSolicitacao(item_concluido)) {
            ...
            case DEMANDA:
                ...
                channel_id = config_.canal_finalizadas;
                break;
            case PEDIDO:
                tipo_str = "Pedido";
                title = is_cancelamento ? "❌ Pedido Cancelado" : "✅ Pedido Finalizado";
                channel_id = config_.canal_pedidos_concluidos;
                break;
            case LEMBRETE:
                tipo_str = "Lembrete";
                db_.removeSolicitacao(codigo);
                cmdHandler_.replyAndDelete(event, dpp::message("✅ Lembrete `" + std::to_string(codigo) + "` finalizado com sucesso!"));
                return;
            }
            ...
            if (channel_id != 0) {
                ...
                if (item_concluido.tipo == DEMANDA) {
                    embed.add_field("Prazo de Entrega", item_concluido.prazo, true);
                }

                if (!is_cancelamento) {
                    if (observacao != "Nenhuma.") {
                        embed.add_field("Observação", observacao, false);
                    }
            ...
            event.reply(dpp::message("❌ Erro ao atualizar a solicitação no banco de dados.").set_flags(dpp::m_ephemeral));
        }
    }
    else {
...
void SolicitacaoCommands::handle_cancelar_demanda(const dpp::slashcommand_t& event) {
    ...
    if (item.tipo != DEMANDA) {
        event.reply(dpp::message("❌ Este comando é apenas para cancelar **demandas**.").set_flags(dpp::m_ephemeral));
        return;
    }

    if (item.status != "pendente") {
        event.reply(dpp::message("❌ Esta demanda não pode ser cancelada (Status: " + item.status + ").").set_flags(dpp::m_ephemeral));
        return;
    }
    ...
    if (db_.addOrUpdateSolicitacao(item)) {
        dpp::embed embed = dpp::embed().set_color(dpp::colors::red)
            .set_title("❌ Demanda Cancelada")
            .add_field("Código", std::to_string(item.id), false)
            .add_field("Responsável", "<@" + std::to_string(item.id_usuario_responsavel) + ">", true)
            .add_field("Cancelado por", event.command.get_issuing_user().get_mention(), true)
            .add_field("Demanda", item.texto, false)
            .add_field("Prazo (Original)", item.prazo, true);
        ...
        event.reply(dpp::message("❌ Erro ao salvar a demanda cancelada no banco de dados.").set_flags(dpp::m_ephemeral));
    }
}
...
void SolicitacaoCommands::handle_limpar_demandas(const dpp::slashcommand_t& event) {
    ...
    auto solicitacoes_map = db_.getSolicitacoes();
    std::vector<uint64_t> ids_para_remover;

    for (auto const& [id, sol] : solicitacoes_map) {
        if (sol.status == "pendente" || sol.tipo == LEMBRETE) {
            ids_para_remover.push_back(id);
        }
    }

    if (ids_para_remover.empty()) {
        event.reply(dpp::message("🧹 Nenhuma solicitação ativa (pendente) encontrada para limpar.").set_flags(dpp::m_ephemeral));
        return;
    }

    int count = 0;
    for (uint64_t id : ids_para_remover) {
        if (db_.removeSolicitacao(id)) {
            count++;
        }
    }
...
void SolicitacaoCommands::handle_lista_demandas(const dpp::slashcommand_t& event) {
    ...
    for (const auto& [id, solicitacao] : todas_solicitacoes) {
        if (solicitacao.id_usuario_responsavel == user_id) {
            ...
            else if (solicitacao.status == status_filtro) {
                solicitacoes_do_usuario.push_back(solicitacao);
            }
        }
    }
    ...
    std::sort(solicitacoes_do_usuario.begin(), solicitacoes_do_usuario.end(), [](const PaginatedItem& a, const PaginatedItem& b) {
        const auto& sol_a = std::get<Solicitacao>(a);
        const auto& sol_b = std::get<Solicitacao>(b);
        ...
        [this, state, needsPagination, event](const dpp::confirmation_callback_t& cb) mutable {
            if (!cb.is_error()) {
                if (needsPagination) {
                    ...
                    // NÃO precisa de paginação, deletar após 60s
                    ...
                    bot_.start_timer([this, event_copy](dpp::timer timer_handle) mutable {
                        event_copy.delete_original_response([](const dpp::confirmation_callback_t& delete_cb) {
                            if (delete_cb.is_error() && delete_cb.get_error().code != 10062) { // Ignora "Unknown Interaction"
                                Utils::log_to_file("Falha ao auto-deletar /lista_demandas: " + delete_cb.get_error().message);
                            }
                            });
...
void SolicitacaoCommands::addCommandDefinitions(std::vector<dpp::slashcommand>& commands, dpp::snowflake bot_id, const BotConfig& config) {
    ...
    // --- COMANDO /lista_demandas (ATUALIZADO) ---
    ...
    lista_cmd.add_option( // <-- OPÇÃO ADICIONADA
