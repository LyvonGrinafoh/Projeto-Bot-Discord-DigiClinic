...
SolicitacaoCommands::SolicitacaoCommands(dpp::cluster& bot, DatabaseManager& db, const BotConfig& config, CommandHandler& handler) :
...
void SolicitacaoCommands::handle_demanda_pedido(const dpp::slashcommand_t& event) {
    ...
    if (db_.addOrUpdateSolicitacao(nova_solicitacao)) {
        ...
        bot_.direct_message_create(responsavel.id, dm_message);
...
void SolicitacaoCommands::handle_finalizar_solicitacao(const dpp::slashcommand_t& event) {
    ...
    if (item_opt) {
        ...
        if (db_.removeSolicitacao(codigo)) {
            ...
            case PEDIDO:
                tipo_str = "Pedido";
                title = (command_name == "cancelar_pedido") ? "❌ Pedido Cancelado" : "✅ Pedido Finalizado";
            ...
            if (channel_id != 0) {
                dpp::embed embed = dpp::embed()
                    .set_color((command_name == "cancelar_pedido") ? dpp::colors::red : dpp::colors::dark_green)
                    .set_title(title)
                    ...
                    .add_field((command_name == "cancelar_pedido" ? "Cancelado por" : "Finalizado por"), event.command.get_issuing_user().get_mention(), true)
            ...
            event.reply(tipo_str + " `" + std::to_string(codigo) + "` " + (command_name == "cancelar_pedido" ? "cancelado" : "finalizado") + " com sucesso!");
...
void SolicitacaoCommands::handle_lista_demandas(const dpp::slashcommand_t& event) {
    ...
    std::string nome_usuario = (usuario_alvo.id != 0) ? usuario_alvo.username : std::to_string(user_id);
    ...
    dpp::embed embed;
    embed.set_color(dpp::colors::white).set_title("📋 Solicitações Pendentes de " + nome_usuario);
    if (!lista_texto.empty()) {
        if (lista_texto.length() > 4000) {
            lista_texto = lista_texto.substr(0, 4000) + "\n... (lista muito longa para exibir)";
        }
