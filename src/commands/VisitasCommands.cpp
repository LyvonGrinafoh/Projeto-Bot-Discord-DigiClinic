...
void VisitasCommands::handle_finalizar_visita(const dpp::slashcommand_t& event) {
    ...
    if (visita_opt) {
        ...
        if (visita.status != "agendada") {
            event.reply(dpp::message("❌ Esta visita não está agendada (Status: " + visita.status + ").").set_flags(dpp::m_ephemeral));
            return;
        }
        ...
        if (db_.addOrUpdateVisita(visita)) {
            ...
            bot_.message_create(dpp::message(config_.canal_visitas, embed));
            cmdHandler_.replyAndDelete(event, dpp::message("✅ Visita marcada como realizada!"));
        }
        else {
            event.reply(dpp::message("❌ Erro ao atualizar a visita.").set_flags(dpp::m_ephemeral));
        }
    }
    else {
...
void VisitasCommands::handle_lista_visitas(const dpp::slashcommand_t& event) {
    ...
    std::sort(filtered_items.begin(), filtered_items.end(), [](const PaginatedItem& a, const PaginatedItem& b) {
        if (std::holds_alternative<Visita>(a) && std::holds_alternative<Visita>(b)) {
    ...
    if (visita_ptr) {
        ...
        if (auto val_ptr = std::get_if<std::string>(&data_param)) {
            if (*val_ptr != visita.data) {
                if (Utils::validarFormatoData(*val_ptr)) {
                    visita.data = *val_ptr;
                    alterado = true;
                }
                else {
                    event.reply(dpp::message("❌ Data inválida. Use o formato DD/MM/AAAA. Nenhuma alteração foi salva.").set_flags(dpp::m_ephemeral));
                    return;
                }
            }
        }
        ...
        if (alterado) {
            if (db_.saveVisitas()) {
                cmdHandler_.replyAndDelete(event, dpp::message("✅ Visita `" + std::to_string(codigo) + "` atualizada com sucesso!"));
            }
            else {
                event.reply(dpp::message("❌ Erro ao salvar as alterações.").set_flags(dpp::m_ephemeral));
            }
        }
        else {
            event.reply(dpp::message("⚠️ Nenhuma alteração foi feita.").set_flags(dpp::m_ephemeral));
        }
    }
    else {
        event.reply(dpp::message("❌ Visita não encontrada.").set_flags(dpp::m_ephemeral));
    }
}
...
void VisitasCommands::addCommandDefinitions(std::vector<dpp::slashcommand>& commands, dpp::snowflake bot_id) {
    ...
    visitas_cmd.add_option(dpp::command_option(dpp::co_string, "telefone", "Telefone de contato (opcional).", false));
    visitas_cmd.add_option(dpp::command_option(dpp::co_string, "observacao", "Observações adicionais (opcional).", false));
    ...
    finalizar_visita_cmd.add_option(dpp::command_option(dpp::co_string, "relatorio", "Breve relatório da visita.", true));
    ...
    lista_visitas_cmd.add_option(dpp::command_option(dpp::co_string, "status", "Filtrar por status (padrão: agendada).", false)
        .add_choice(dpp::command_option_choice("Agendadas", std::string("agendada")))
        .add_choice(dpp::command_option_choice("Realizadas", std::string("realizada")))
    ...
    dpp::slashcommand modificar_visita_cmd("modificar_visita", "Altera dados de uma visita agendada.", bot_id);
    ...
    modificar_visita_cmd.add_option(dpp::command_option(dpp::co_string, "data", "Nova data (DD/MM/AAAA).", false));
    modificar_visita_cmd.add_option(dpp::command_option(dpp::co_string, "horario", "Novo horário.", false));
    modificar_visita_cmd.add_option(dpp::command_option(dpp::co_string, "unidade", "Nova unidade.", false)
        .add_choice(dpp::command_option_choice("Tatuapé", std::string("Tatuapé")))
        .add_choice(dpp::command_option_choice("Campo Belo", std::string("Campo Belo"))));
