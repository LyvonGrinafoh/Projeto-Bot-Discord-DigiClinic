...
void ReportGenerator::gerarPlanilhaLeads(const dpp::slashcommand_t& event) {
    std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y"); std::string filename = "Planilha_Leads_" + ss.str() + ".xlsx"; std::remove(filename.c_str());
    ...
    const auto& leads_map = db_.getLeads(); std::vector<Lead> sorted_leads; for (const auto& pair : leads_map) { sorted_leads.push_back(pair.second); }
    ...
    try {
        ...
        event.edit_original_response(msg, [this, event_copy, captured_filename](const dpp::confirmation_callback_t& cb) {
            if (!cb.is_error()) {
                ...
                bot_.start_timer([captured_filename](dpp::timer timer_handle) {
...
void ReportGenerator::gerarPlanilhaCompras(const dpp::slashcommand_t& event) {
    std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y"); std::string filename = "Lista_Compras_" + ss.str() + ".xlsx"; std::remove(filename.c_str());
    lxw_workbook* workbook = workbook_new(filename.c_str()); if (!workbook) { event.edit_original_response(dpp::message("Erro ao criar o arquivo Excel para Compras.").set_flags(dpp::m_ephemeral)); Utils::log_to_file("ERRO FATAL: workbook_new falhou para " + filename); return; } lxw_worksheet* worksheet = workbook_add_worksheet(workbook, "Compras"); if (!worksheet) { event.edit_original_response(dpp::message("Erro ao criar a planilha Excel para Compras.").set_flags(dpp::m_ephemeral)); Utils::log_to_file("ERRO FATAL: workbook_add_worksheet falhou para Compras"); workbook_close(workbook); return; }
    worksheet_set_column(worksheet, 0, 0, 15, NULL); worksheet_set_column(worksheet, 1, 1, 40, NULL); worksheet_set_column(worksheet, 2, 2, 20, NULL); worksheet_set_column(worksheet, 3, 3, 20, NULL); worksheet_set_column(worksheet, 4, 4, 12, NULL); worksheet_set_column(worksheet, 5, 5, 50, NULL); worksheet_set_column(worksheet, 6, 6, 25, NULL); worksheet_set_column(worksheet, 7, 7, 25, NULL);
    lxw_format* header_format = workbook_add_format(workbook); format_set_bold(header_format); lxw_format* wrap_format = workbook_add_format(workbook); format_set_text_wrap(wrap_format);
    const char* headers[] = { "ID", "Item", "Local Sugerido", "Unidade Destino", "Quantidade", "Observação", "Solicitado Por", "Data da Solicitação" }; for (int i = 0; i < 8; ++i) { worksheet_write_string(worksheet, 0, i, headers[i], header_format); }
    const auto& compras_map = db_.getCompras(); int row = 1;
    for (const auto& [id, compra] : compras_map) { worksheet_write_number(worksheet, row, 0, compra.id, NULL); worksheet_write_string(worksheet, row, 1, compra.item.c_str(), NULL); worksheet_write_string(worksheet, row, 2, compra.local_compra.c_str(), NULL); worksheet_write_string(worksheet, row, 3, compra.unidade_destino.c_str(), NULL); if (compra.quantidade > 0) { worksheet_write_number(worksheet, row, 4, compra.quantidade, NULL); } else { worksheet_write_string(worksheet, row, 4, "N/A", NULL); } worksheet_write_string(worksheet, row, 5, compra.observacao.c_str(), wrap_format); worksheet_write_string(worksheet, row, 6, compra.solicitado_por.c_str(), NULL); worksheet_write_string(worksheet, row, 7, compra.data_solicitacao.c_str(), NULL); row++; }
    if (workbook_close(workbook) != LXW_NO_ERROR) { event.edit_original_response(dpp::message("Erro ao finalizar e salvar o arquivo Excel para Compras.").set_flags(dpp::m_ephemeral)); Utils::log_to_file("ERRO: workbook_close falhou para " + filename); return; }
    dpp::message msg; msg.set_content("Aqui está a planilha da lista de compras!");
    try {
        msg.add_file(filename, dpp::utility::read_file(filename));
        dpp::slashcommand_t event_copy = event;
        std::string captured_filename = filename;
        event.edit_original_response(msg, [this, event_copy, captured_filename](const dpp::confirmation_callback_t& cb) {
            if (!cb.is_error()) {
                bot_.start_timer([event_copy](dpp::timer th) mutable { event_copy.delete_original_response([](const auto& dcb) { if (dcb.is_error() && dcb.get_error().code != 10062) { Utils::log_to_file("Falha ao deletar resposta (planilha compras): " + dcb.get_error().message); }}); }, 10);
                bot_.start_timer([captured_filename](dpp::timer timer_handle) {
                    if (std::remove(captured_filename.c_str()) == 0) {
                        Utils::log_to_file("Arquivo de planilha removido com sucesso: " + captured_filename);
                    }
            ...
            else { Utils::log_to_file("Falha ao editar resposta original com planilha compras: " + cb.get_error().message); }
            });
    }
    catch (const dpp::exception& e) { event.edit_original_response(dpp::message("Erro ao ler ou anexar o arquivo da planilha de compras: " + std::string(e.what()))); Utils::log_to_file("ERRO: Falha ao enviar planilha de compras: " + std::string(e.what())); }
}

void ReportGenerator::gerarPlanilhaVisitas(const dpp::slashcommand_t& event) {
    std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y"); std::string filename = "Planilha_Visitas_" + ss.str() + ".xlsx"; std::remove(filename.c_str());
    ...
    const auto& visitas_map = db_.getVisitas(); std::vector<Visita> sorted_visitas; for (const auto& pair : visitas_map) { sorted_visitas.push_back(pair.second); } int row = 1;
    for (const auto& visita : sorted_visitas) { bool cancelada = (visita.observacoes.find("Visita Cancelada") != std::string::npos); lxw_format* row_format = cancelada ? cancelled_format : NULL; lxw_format* obs_format = cancelada ? cancelled_wrap_format : wrap_format; worksheet_write_number(worksheet, row, 0, visita.id, row_format); worksheet_write_string(worksheet, row, 1, cancelada ? "Cancelada" : "Agendada", row_format); worksheet_write_string(worksheet, row, 2, visita.doutor.c_str(), row_format); worksheet_write_string(worksheet, row, 3, visita.area.c_str(), row_format); worksheet_write_string(worksheet, row, 4, visita.data.c_str(), row_format); worksheet_write_string(worksheet, row, 5, visita.horario.c_str(), row_format); worksheet_write_string(worksheet, row, 6, visita.unidade.c_str(), row_format); worksheet_write_string(worksheet, row, 7, visita.telefone.c_str(), row_format); worksheet_write_string(worksheet, row, 8, visita.quem_marcou_nome.c_str(), row_format); worksheet_write_string(worksheet, row, 9, visita.observacoes.c_str(), obs_format); row++; }
    ...
    try {
        ...
        event.edit_original_response(msg, [this, event_copy, captured_filename](const dpp::confirmation_callback_t& cb) {
            if (!cb.is_error()) {
                bot_.start_timer([event_copy](dpp::timer th) mutable { event_copy.delete_original_response([](const auto& dcb) { if (dcb.is_error() && dcb.get_error().code != 10062) { Utils::log_to_file("Falha ao deletar resposta (planilha visitas): " + dcb.get_error().message); }}); }, 10);
                bot_.start_timer([captured_filename](dpp::timer timer_handle) {
                    if (std::remove(captured_filename.c_str()) == 0) {
                        Utils::log_to_file("Arquivo de planilha removido com sucesso: " + captured_filename);
                    }
