...
void ReportGenerator::gerarPlanilhaLeads(const dpp::slashcommand_t& event, std::optional<int64_t> mes, std::optional<int64_t> ano) {
    ...
    std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y"); std::string filename = "Planilha_Leads" + filename_sufixo + "_" + ss.str() + ".xlsx"; std::remove(filename.c_str());
    ...
    for (const auto& pair : leads_map) { if (checkDateFilter(pair.second.data_contato, mes, ano)) { sorted_leads.push_back(pair.second); } }
    ...
    for (const auto& lead : sorted_leads) {
        ...
        worksheet_write_number(worksheet, row, 0, lead.id, NULL); worksheet_write_string(worksheet, row, 1, lead.origem.c_str(), NULL); worksheet_write_string(worksheet, row, 2, lead.data_contato.c_str(), NULL); worksheet_write_string(worksheet, row, 3, lead.hora_contato.c_str(), NULL); worksheet_write_string(worksheet, row, 4, lead.nome.c_str(), NULL); worksheet_write_string(worksheet, row, 5, lead.contato.c_str(), NULL); worksheet_write_string(worksheet, row, 6, lead.area_atuacao.c_str(), NULL); worksheet_write_string(worksheet, row, 7, lead.conhece_formato ? "Sim" : "Nao", NULL); worksheet_write_string(worksheet, row, 8, lead.veio_campanha ? "Sim" : "Nao", NULL); worksheet_write_string(worksheet, row, 9, lead.respondido ? "Sim" : "Nao", NULL); worksheet_write_string(worksheet, row, 10, lead.data_hora_resposta.c_str(), NULL); worksheet_write_string(worksheet, row, 11, lead.problema_contato ? "Sim" : "Nao", NULL); worksheet_write_string(worksheet, row, 12, lead.observacoes.c_str(), wrap_format);
        ...
        worksheet_write_string(worksheet, row, 13, lead.status_conversa.c_str(), status_format); worksheet_write_string(worksheet, row, 14, lead.status_followup.c_str(), NULL); worksheet_write_string(worksheet, row, 15, lead.unidade.c_str(), NULL); worksheet_write_string(worksheet, row, 16, lead.convidou_visita ? "Sim" : "Nao", NULL); worksheet_write_string(worksheet, row, 17, lead.agendou_visita ? "Sim" : "Nao", NULL); worksheet_write_string(worksheet, row, 18, lead.tipo_fechamento.c_str(), NULL); worksheet_write_string(worksheet, row, 19, lead.teve_adicionais ? "Sim" : "Nao", NULL); worksheet_write_number(worksheet, row, 20, lead.valor_fechamento, money_format);
        if (!lead.print_final_conversa.empty()) { if (lead.print_final_conversa.rfind("lead_prints", 0) == 0 && std::filesystem::exists(lead.print_final_conversa)) { worksheet_set_row(worksheet, row, 150, NULL); lxw_image_options options = { .x_scale = 0.2, .y_scale = 0.2, .object_position = LXW_OBJECT_MOVE_DONT_SIZE }; worksheet_insert_image_opt(worksheet, row, 21, lead.print_final_conversa.c_str(), &options); has_image = true; } else if (lead.print_final_conversa.rfind("http", 0) == 0) { worksheet_write_url(worksheet, row, 21, lead.print_final_conversa.c_str(), NULL); } else { worksheet_write_string(worksheet, row, 21, lead.print_final_conversa.c_str(), NULL); } }
        else { worksheet_write_string(worksheet, row, 21, "", NULL); }
        if (!has_image) { worksheet_set_row(worksheet, row, LXW_DEF_ROW_HEIGHT, NULL); }
        worksheet_write_string(worksheet, row, 22, lead.criado_por.empty() ? "N/A" : lead.criado_por.c_str(), NULL); row++;
    }
    if (row == 1) { worksheet_write_string(worksheet, 1, 0, "Nenhum lead encontrado para o filtro selecionado.", NULL); }
    ...
    dpp::message msg; msg.set_content("Aqui está a planilha de leads atualizada!"); try { msg.add_file(filename, dpp::utility::read_file(filename)); dpp::slashcommand_t event_copy = event; std::string captured_filename = filename; event.edit_original_response(msg, [this, event_copy, captured_filename](const dpp::confirmation_callback_t& cb) { if (!cb.is_error()) { bot_.start_timer([this, event_copy](dpp::timer th) mutable { event_copy.delete_original_response([](const dpp::confirmation_callback_t& delete_cb) { if (delete_cb.is_error() && delete_cb.get_error().code != 10062) { Utils::log_to_file("Falha ao deletar resposta (planilha leads): " + delete_cb.get_error().message); } }); bot_.stop_timer(th); }, 10); bot_.start_timer([this, captured_filename](dpp::timer timer_handle) { if (std::remove(captured_filename.c_str()) == 0) { Utils::log_to_file("Arquivo de planilha removido com sucesso: " + captured_filename); } else { Utils::log_to_file("AVISO: Falha ao remover arquivo de planilha: " + captured_filename); } bot_.stop_timer(timer_handle); }, 900); } else { Utils::log_to_file("Falha ao editar resposta original com planilha leads: " + cb.get_error().message); } }); }
...
void ReportGenerator::gerarPlanilhaVisitas(const dpp::slashcommand_t& event, std::optional<int64_t> mes, std::optional<int64_t> ano) {
    std::string filename_sufixo = getFilenameSufixo(mes, ano); std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y"); std::string filename = "Planilha_Visitas" + filename_sufixo + "_" + ss.str() + ".xlsx"; std::remove(filename.c_str());
    ...
    const auto& visitas_map = db_.getVisitas(); std::vector<Visita> sorted_visitas; for (const auto& pair : visitas_map) { if (checkDateFilter(pair.second.data, mes, ano)) { sorted_visitas.push_back(pair.second); } } int row = 1;
    for (const auto& visita : sorted_visitas) { lxw_format* row_format = (visita.status == "cancelada") ? cancelled_format : NULL; lxw_format* obs_format = (visita.status == "cancelada") ? cancelled_wrap_format : wrap_format; worksheet_write_number(worksheet, row, 0, visita.id, row_format); worksheet_write_string(worksheet, row, 1, visita.status.c_str(), row_format); worksheet_write_string(worksheet, row, 2, visita.doutor.c_str(), row_format); worksheet_write_string(worksheet, row, 3, visita.area.c_str(), row_format); worksheet_write_string(worksheet, row, 4, visita.data.c_str(), row_format); worksheet_write_string(worksheet, row, 5, visita.horario.c_str(), row_format); worksheet_write_string(worksheet, row, 6, visita.unidade.c_str(), row_format); worksheet_write_string(worksheet, row, 7, visita.telefone.c_str(), row_format); worksheet_write_string(worksheet, row, 8, visita.quem_marcou_nome.c_str(), row_format); worksheet_write_string(worksheet, row, 9, visita.observacoes.c_str(), obs_format); row++; }
    if (row == 1) { worksheet_write_string(worksheet, 1, 0, "Nenhuma visita encontrada para o filtro selecionado.", NULL); }
    ...
    dpp::message msg; msg.set_content("Aqui está a planilha de visitas agendadas!"); try { msg.add_file(filename, dpp::utility::read_file(filename)); dpp::slashcommand_t event_copy = event; std::string captured_filename = filename; event.edit_original_response(msg, [this, event_copy, captured_filename](const dpp::confirmation_callback_t& cb) { if (!cb.is_error()) { bot_.start_timer([this, event_copy](dpp::timer th) mutable { event_copy.delete_original_response([](const auto& dcb) { if (dcb.is_error() && dcb.get_error().code != 10062) { Utils::log_to_file("Falha ao deletar resposta (planilha visitas): " + dcb.get_error().message); }}); bot_.stop_timer(th); }, 10); bot_.start_timer([this, captured_filename](dpp::timer timer_handle) { if (std::remove(captured_filename.c_str()) == 0) { Utils::log_to_file("Arquivo de planilha removido com sucesso: " + captured_filename); } else { Utils::log_to_file("AVISO: Falha ao remover arquivo de planilha: " + captured_filename); } bot_.stop_timer(timer_handle); }, 900); } else { Utils::log_to_file("Falha ao editar resposta original com planilha visitas: " + cb.get_error().message); } }); }
...
void ReportGenerator::gerarPlanilhaDemandas(const dpp::slashcommand_t& event, std::optional<int64_t> mes, std::optional<int64_t> ano, std::optional<dpp::snowflake> usuario_filtro) {
    std::string filename_sufixo = getFilenameSufixo(mes, ano);
    if (usuario_filtro.has_value()) { filename_sufixo += "_User-" + std::to_string(usuario_filtro.value()); }

    std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y"); std::string filename = "Planilha_Demandas" + filename_sufixo + "_" + ss.str() + ".xlsx"; std::remove(filename.c_str());
    ...
    worksheet_set_column(worksheet, 6, 6, 20, NULL); // Data Criacao (NOVO)
    ...
    for (const auto& [id, sol] : solicitacoes_map) {
        if (usuario_filtro.has_value() && sol.id_usuario_responsavel != usuario_filtro.value()) {
            continue;
        }

        if (!sol.data_criacao.empty()) {
            if (!checkTimestampFilter(sol.data_criacao, mes, ano)) {
                continue;
            }
        }
        else if (mes.has_value() || ano.has_value()) {
            continue;
        }
        ...
        else if (sol.prioridade == 0) prioridade_str = "Baixa";
...
void ReportGenerator::gerarPlanilhaReposicao(const dpp::slashcommand_t& event) {
    std::time_t tt = std::time(nullptr); std::tm* agora = std::localtime(&tt); std::stringstream ss; ss << std::put_time(agora, "%d-%m-%Y");
    std::string filename = "Lista_Reposicao_" + ss.str() + ".xlsx";
    std::remove(filename.c_str());

    lxw_workbook* workbook = workbook_new(filename.c_str()); if (!workbook) { event.edit_original_response(dpp::message("Erro ao criar o arquivo Excel.").set_flags(dpp::m_ephemeral)); return; }
    lxw_worksheet* worksheet = workbook_add_worksheet(workbook, "Reposicao"); if (!worksheet) { event.edit_original_response(dpp::message("Erro ao criar a planilha.").set_flags(dpp::m_ephemeral)); workbook_close(workbook); return; }

    worksheet_set_column(worksheet, 0, 0, 30, NULL); // Item
    worksheet_set_column(worksheet, 1, 1, 15, NULL); // Qtd Atual
    worksheet_set_column(worksheet, 2, 2, 15, NULL); // Qtd Minima
    worksheet_set_column(worksheet, 3, 3, 20, NULL); // Local
    worksheet_set_column(worksheet, 4, 4, 15, NULL); // Unidade

    lxw_format* header_format = workbook_add_format(workbook); format_set_bold(header_format);
    lxw_format* alert_format = workbook_add_format(workbook); format_set_bg_color(alert_format, LXW_COLOR_RED); format_set_font_color(alert_format, LXW_COLOR_WHITE);

    const char* headers[] = { "Item", "Qtd Atual", "Qtd Minima", "Local", "Unidade" };
    for (int i = 0; i < 5; ++i) { worksheet_write_string(worksheet, 0, i, headers[i], header_format); }
    ...
    for (const auto& [id, item] : estoque_map) {
        if (item.quantidade <= item.quantidade_minima) {
            worksheet_write_string(worksheet, row, 0, item.nome_item.c_str(), NULL);
            worksheet_write_number(worksheet, row, 1, item.quantidade, alert_format);
            worksheet_write_number(worksheet, row, 2, item.quantidade_minima, NULL);
            worksheet_write_string(worksheet, row, 3, item.local_estoque.c_str(), NULL);
            worksheet_write_string(worksheet, row, 4, item.unidade.c_str(), NULL);
            row++;
        }
    }

    if (row == 1) {
        worksheet_write_string(worksheet, 1, 0, "Nenhum item abaixo do estoque mínimo.", NULL);
    }

    if (workbook_close(workbook) != LXW_NO_ERROR) { event.edit_original_response(dpp::message("Erro ao salvar o arquivo.").set_flags(dpp::m_ephemeral)); return; }

    dpp::message msg; msg.set_content("⚠️ Aqui está a lista de itens que precisam de reposição!");
    try {
        msg.add_file(filename, dpp::utility::read_file(filename));
        dpp::slashcommand_t event_copy = event;
        std::string captured_filename = filename;
        event.edit_original_response(msg, [this, event_copy, captured_filename](const dpp::confirmation_callback_t& cb) {
            if (!cb.is_error()) {
                bot_.start_timer([this, event_copy](dpp::timer th) mutable { event_copy.delete_original_response([](const auto& dcb) {}); bot_.stop_timer(th); }, 10);
                bot_.start_timer([this, captured_filename](dpp::timer timer_handle) { std::remove(captured_filename.c_str()); bot_.stop_timer(timer_handle); }, 900);
            }
            });
    }
